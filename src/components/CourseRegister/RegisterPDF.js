...
export default function RegisterPDF({activeStep, handleNext, handleBack}) {
  ...
  const handleSubmitForm = async (values) => {
    const {
      ageStudent, certificateId, emailP, gradeId, nameP, programId, schoolId, phoneNoP, subjects, tutoringAddress,
    } = values;
  ...
  const renderValidationSchema = () => {
    ...
    if (key === 'schoolId') {
      validation = {
        ...validation,
        schoolId: Yup.string().required(1, 'Vui lòng chọn ít nhất một trường học'), // ageStudent: Yup.string().required('Vui lòng chọn lớp học'),
      };
    }
    ...
    if (key === 'certificateId') {
      validation = {
        ...validation,
        certificateId: Yup.string().required(1, 'Vui lòng chọn ít nhất một chứng chỉ'), // ageStudent: Yup.string().required('Vui lòng chọn lớp học'),
      };
    }
      ...
      >
        {({values, errors, touched, setFieldValue, isValid}) => (<Form>
            <Box>
              ...
              {activeStep === 1 ? (<Box
                ...
                >
                  ...
                  {touched.nameP && errors.nameP && (<ErrorMessage
                      name="nameP"
                      className="text-red-500"
                      component="div"
                    />)}
            ...
            <Box className="flex justify-center">
              ...
              {activeStep == 1 ? (<Button
                  type="submit"
                  className={`${btnClass} ${isValid ? 'ca-primary-bg-color text-white ' : 'pointer-events-none cursor-not-allowed bg-gray-300 text-gray-100'}`}
                >
                  Đăng ký
                </Button>) : ('')}
            </Box>
